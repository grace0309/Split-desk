...
html_doc.search('.tgme_widget_message_bubble').each do |post|
  image_element = post.search('.tgme_widget_message_photo_wrap')
    numbered_link = image_element.attribute('href')
    if numbered_link.nil? == false
      number = numbered_link.value.gsub("https://t.me/goodlobang/", "")
  ...
  content = text_element.text.strip.gsub(/(Telegram: @goodlobang)/, '').gsub(/More info: @goodlobangpolice/, '').gsub(/Join now: @goodlobangpolice/,'').gsub(/More info: @goodlobangpolice/, '').gsub(/@goodlobang/,'')
  ...
  $next_number = numbers_array[0]
end

creatingDeals("https://t.me/s/goodlobang")
creatingDeals("http://t.me/s/goodlobang?before=#{$next_number}")
creatingDeals("http://t.me/s/goodlobang?before=#{$next_number}")
# end
